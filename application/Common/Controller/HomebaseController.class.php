...
class HomebaseController extends AppframeController {
	...
	//查询新闻分类
	function news_cate(){
        $news_cate =M('terms')->field("term_id,name")->where(array('parent'=>1,'status'=>1))->select();
        $this->assign("news_cate",$news_cate);
    }

    //查询产品分类
    function solution_cate(){
        $solution_cate =M('terms')->field("term_id,name")->where(array('parent'=>2,'status'=>1))->select();
        $this->assign("solution_cate",$solution_cate);
    }
